...
class LangChainToolAdapter(LangChainBaseTool):
    ...
    def __init__(self, **data: Any) -> None:
        """Initialize adapter with OpenCode tool."""
        super().__init__(**data)
        # Set name and description from wrapped tool
        if self.opencode_tool:
            object.__setattr__(self, "name", self.opencode_tool.name)
            object.__setattr__(self, "description", self.opencode_tool.description)
...
def adapt_tools_for_opencode(tools: list[LangChainBaseTool]) -> list[OpenCodeToolAdapter]:
    """
