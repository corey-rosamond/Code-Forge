"""Configuration loader for OpenCode.
...
from opencode.core import ConfigError, IConfigLoader, get_logger
...
class ConfigLoader(IConfigLoader):
    ...
    1. Defaults (from OpenCodeConfig)
    2. Enterprise settings (/etc/opencode/settings.json)
    3. User settings (~/.opencode/settings.json or .yaml)
    4. Project settings (.opencode/settings.json or .yaml)
    5. Local settings (.opencode/settings.local.json)
    ...
    def __init__(
        ...
        Args:
            user_dir: User configuration directory. Defaults to ~/.opencode
            project_dir: Project configuration directory. Defaults to ./.opencode
            enterprise_dir: Enterprise configuration directory. Defaults to /etc/opencode
        """
        self._user_dir = user_dir or Path.home() / ".opencode"
        self._project_dir = project_dir or Path.cwd() / ".opencode"
        self._enterprise_dir = enterprise_dir or Path("/etc/opencode")
    ...
    def config(self) -> OpenCodeConfig:
        ...
        Returns:
            Current OpenCodeConfig instance.
        """
        with self._lock:
    ...
    def load_all(self) -> OpenCodeConfig:
        ...
        Returns:
            Validated OpenCodeConfig with all sources merged.

        Raises:
            ConfigError: If configuration cannot be loaded or validated.
        """
        # Start with defaults
        config: dict[str, Any] = OpenCodeConfig().model_dump()
    ...
    def add_observer(self, callback: Callable[[OpenCodeConfig], None]) -> None:
        ...
        Args:
            callback: Function to call when configuration changes.
                      Receives the new OpenCodeConfig instance.
        """
